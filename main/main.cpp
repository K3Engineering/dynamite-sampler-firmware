...
void setup_ble() {
  ...
  uint32_t prop = NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::WRITE |
...
void app_main(void) {
  ...
  xTaskCreatePinnedToCore(task_setup_adc, "taskADC_setup", 5000, NULL, 1, NULL,
                          CORE_APP);
  // setup_adc();  // TODO force run this on core 1 to make sure ISR is on core
  // 1
  delay(500);
