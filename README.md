...
Every `idf.py flash` also writes `build/factory_nvs.bin` to the `DynaPersistent`
partition. The image is generated by `generate_factory_nvs.py` from the hardware
revision selected in the active `sdkconfig` and currently contains a single key:
`Factory::board_model` (e.g. `v700L`).

**WARNING**: this rewrites the entire `DynaPersistent` partition on every flash,
erasing the `User` namespace (and anything else stored in `Factory`).
